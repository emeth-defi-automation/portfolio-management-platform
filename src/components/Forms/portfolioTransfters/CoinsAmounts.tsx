...
export default component$<CoinsAmountsProps>(({ batchTransferFormStore, availableStructures }) => {

  return (
    <>
     {
                 batchTransferFormStore.coinsToTransfer.map((structure: any, index: number) => (
                    ...
                    <div class="flex flex-col p-2 custom-border-1">
                      {structure.coins.map((coinObject:any, index:number) => (
                        <div key={`${coinObject.name}${index}`}>
                          {coinObject.coins.map((coin:any, insideIndex:number) => (
                            <FormBadge 
                              class="mb-2"
                              ...
                              input={
                                ...
                                onInput$={(e) => {
                                  ...
                                  const x = batchTransferFormStore.coinsToTransfer.find(item => item.name === structure.name);
                                  const y = x!.coins.find((item => item.wallet === coinObject.wallet));

                                  y!.coins.find(c => c.symbol === coin.symbol)!.amount = target.value;
                                }}
                              />
                              }
                            />
