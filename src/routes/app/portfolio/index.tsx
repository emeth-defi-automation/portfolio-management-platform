...
export default component$(() => {
  ...
  const calculate = $(async () => {
    if (tokenFromAddress.value) {
      ...
      const numerator =
        BigInt(tokenToEstimatedAmount.value) /
        BigInt(BigInt(10) ** BigInt(decimals[0]));
      const denominator = tokenToEstimatedAmount.value
        .toString()
        .substring(
  ...
  useVisibleTask$(async ({ track }) => {
    ...
    const findFromToken = response.find(
      (token) => token.tokenSymbol === tokenFromSymbol.value,
    );
    const findToToken = response.find(
      (token) => token.tokenSymbol === tokenToSymbol.value,
    );

    if (findFromToken?.usdPrice && findToToken?.usdPrice) {
      estimatedSwapFromDolarValue.value =
        `${Number(tokenFromAmount.value) * Number(findFromToken!.usdPrice)}`.substring(
          0,
          5,
        );

      estimatedSwapToDolarValue.value =
        `${Number(estimatedAfterSwapTokenValue.value) * Number(findToToken!.usdPrice)}`.substring(
          0,
          5,
        );
    }
              ...
              <div class="flex flex-col gap-2">
                <Box customClass="!shadow-none flex justify-between p-4 rounded-xl">
                  ...
                  <Select
                    ...
                    options={[
                      {
                        value: tokenFromAddress.value,
                        text: tokenFromSymbol.value,
                      },
                    ]}
                    size="medium"
                    class="h-8 pr-0"
